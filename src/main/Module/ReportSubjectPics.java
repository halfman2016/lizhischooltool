...
public class ReportSubjectPics {
    ...
    private int count;
    private int index;
    public ReportSubjectPics(List <Photopic> photopics) {
        titles.add("拍摄时间");
        titles.add("拍摄人");
        titles.add("专题名");
        titles.add("拍摄说明");
        ...
        for(Photopic photopic:photopics)
        {
            ...
            subjectname=(mdb.getSubject((photopic.getBelongToSubject()).toString())).getSubjectName();
    ...
    {
        //初始化之后调用
        File file=new File("专题照片导出.xls");

        try {
            WritableWorkbook workbook= Workbook.createWorkbook(file);
            WritableSheet sheet=workbook.createSheet("专题照片导出",0);
            ...
            for (int i=0;i<index;i++)
            {
                ...
                sheet.addCell(label);
                label=new Label(2,i+1,subjects.get(i));
                sheet.addCell(label);
                label=new Label(3,i+1,memos.get(i));
                sheet.addCell(label);
                label=new Label(4,i+1,picnames.get(i));
                sheet.addCell(label);
                label=new Label(5,i+1,comments.get(i));
                sheet.addCell(label);
            }
            workbook.write();
            workbook.close();
